...
static PyObject * pymqe_MQCONNX(PyObject *self, PyObject *args) {
  ...
  if(PyDict_Size(user_password)) {
    ...
    csp.CSPUserIdPtr = PyUnicode_AsUTF8AndSize(user, &strSize);
    csp.CSPUserIdLength = strSize;
    csp.CSPPasswordPtr = PyUnicode_AsUTF8AndSize(password, &strSize);
    csp.CSPPasswordLength = strSize;
  }
...
static PyObject *mqputN(int put1Flag, PyObject *self, PyObject *args) {
  ...
  if (!put1Flag) {
//      PyErr_Format(ErrorObj, "msgBufferLength: %d", msgBufferLength);
//      return NULL;
...
static PyObject *pymqe_mqaiExecute(PyObject *self, PyObject *args) {
    ...
    do {
              ...
              for(i = 0; i < nKeys; i++) {
                   ...
                   */
                   if (PyLong_Check(value)) {
                       ...
                       strArg = PyBytes_AsString(PyUnicode_AsASCIIString(value));
        ...
        if(compCode != MQCC_OK) {
//            char str[1024];
//            sprintf(str, "reini war da: %d, %d", compCode, compReason);
//    PyErr_SetString(ErrorObj, str);
//    return NULL;
 ...
 PyMODINIT_FUNC PyInit_pymqe(void) {
   ...
   */
  {
    PyObject *versions = PyList_New(0);
      ...
      PyList_Append(versions, PyUnicode_FromString("8.0.0"));
#endif
#ifdef MQCMDL_LEVEL_801
      PyList_Append(versions, PyUnicode_FromString("8.0.1"));
#endif

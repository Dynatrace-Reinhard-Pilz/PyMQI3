...
class MQOpts(object):
    ...
    def pack(self):
        ...
        # return struct.pack(self.__format.encode(), *args)
...
class gmo(MQOpts):
    ...
    def __init__(self, **kw):
        ...
        if "7.0" in pymqe.__mqlevels__:
            opts += [
                ['Reserved2', 0, MQLONG_TYPE],
                ['MsgHandle', 0, 'q']]
                
        self.debug = False

        MQOpts.__init__(self, tuple(opts), **kw)
...
class RFH2(MQOpts):
    ...
    def __init__(self, **kw):
        ...
        MQOpts.__init__(self, tuple(self.opts), **kw)
        self.debug = False
    ...
    def add_folder(self, folder_data):
        ...
        folder_data = bytes(folder_data, "UTF-8")
        folder_name = None
        ...
        folder_length = len(folder_data.decode())
        ...
        self.opts.append([folder_name + "Length", int(folder_length),
                          MQLONG_TYPE])
        self.opts.append([folder_name, folder_data.decode(), "%is" % folder_length])
        #save the current values
        saved_values = self.get()
        #reinit MQOpts with new fields added
        MQOpts.__init__(self, tuple(self.opts), )
    ...
    def unpack(self, buff, encoding=None):
        ...
        if buff[0:4].decode("UTF-8") != CMQC.MQRFH_STRUC_ID:
            raise PYIFError("RFH2 - StrucId not MQRFH_STRUC_ID. Value: %s" %
                            buff[0:4].decode("UTF-8"))
...
class cd(MQOpts):
    ...
    def __init__(self, **kw):
        ...
        if "6.0" in pymqe.__mqlevels__:
            opts += [['HdrCompList', [0, -1], '2' + MQLONG_TYPE],
...
class sco(MQOpts):
    ...
    def __init__(self, **kw):
        ...
        if "6.0" in pymqe.__mqlevels__:
            opts += [['KeyResetCount', 0, MQLONG_TYPE],
        ...
        if "7.1" in pymqe.__mqlevels__:
            opts += [['CertificateValPolicy', 0, MQLONG_TYPE]]
...
class QueueManager:
    ...
    def connect(self, name):
        ...
        if rv[1]:
            if rv[2] != CMQC.MQRC_ALREADY_CONNECTED:
...
# Connect options suggested by Jaco Smuts (mailto:JSmuts@clover.co.za)
    ...
    def connectWithOptions(self, name, *bwopts, **kw):
        ...
        if bwoptsLen:
            ...
            # New style args
            for k in kw.keys():
                if k not in ('opts', 'cd', 'sco', 'user', 'password'):
                    raise TypeError('Invalid option: %s' % k)
    ...
    def put1(self, qDesc, msg, *opts):
        ...
        mDesc, putOpts = commonQArgs(*opts)
        if putOpts == None:
            putOpts = pmo()
...
def makeQDesc(qDescOrString):
    ...
    if isinstance(qDescOrString, str):
...
class Queue:
        ...
        if len(opts) >= 3:
            if opts[2] is not None:
                if not isinstance(opts[2], list):
                    raise TypeError("Third item of opts should " \
                                               "be a list.")
    ...
    def get(self, maxLength = None, *opts):
        ...
        mDesc, getOpts = commonQArgs(*opts)
        if getOpts == None:
            getOpts = gmo()
        # If queue open was deferred, open it for put now
        if not self.__qHandle:
